...
async function createContract(pid){try{const r=await api('POST','/crm/contracts',{project_id:pid,prepayment:parseFloat(document.getElementById('ctPrepay').value)||0,notes:document.getElementById('ctNotes').value});closeModal();alert('Договор '+r.contract_number+' создан!');window.open('/api/crm/contracts/'+r.id+'/html','_blank');navigate('contracts')}catch(e){alert(e.message)}}
